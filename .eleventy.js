...
module.exports = function (eleventyConfig) {
  ...
  eleventyConfig.addPlugin(elasticSearch, {
		...
		document: (item) => {
			...
			// Create an index of all the known types. The order is important as the first
			// matching predicate will be used.
			const definedTypes = [
				require('./_includes/es_index/issue')(eleventyConfig),
				require('./_includes/es_index/article')(eleventyConfig),
				require('./_includes/es_index/page')(eleventyConfig)
			]

			const tmpl = definedTypes.find( tmplConfig => tmplConfig.predicate(item) )
			
			if (tmpl) {
				console.log( `Template found: ${tmpl.type}`)
				return tmpl.buildIndexEntry(item)
			}
			
			// Error...
			throw new Error("No template matched.")
		}
	})
